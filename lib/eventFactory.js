...
const EventQueries = require('./rawQueries').EventFactoryQueries;
let instance;
...
class EventFactory extends BaseFactory {
     ...
     * @param  {Array}    config.groupEventId     Group event ID to get build statuses for
     * @return {Promise}
     */
    getBuildStatuses(config) {
        const queryConfig = {
            queries: [
                { dbType: 'postgres', query: EventQueries.statusesQuery },
                { dbType: 'sqlite', query: EventQueries.statusesQuery },
                { dbType: 'mysql', query: EventQueries.statusesQueryMySql }
            ],
            readOnly: true,
            replacements: {
