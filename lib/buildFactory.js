...
class BuildFactory extends BaseFactory {
     ...
     */
    create(config) {
        const [username, start, meta, jobId, configPipelineSha, clusterEnv] = config;
        const modelConfig = config;
        const number = Date.now();
        const displayLabel = this.scm.getDisplayName(config);
        const displayName = displayLabel ? `${displayLabel}:${username}` : username;

        // if it did not go through eventFactory Create to set clusterEnv
        if (!clusterEnv || Object.keys(clusterEnv).length === 0) {
        ...
        return factory.get(jobId)
            .then((job) => {
                ...
                return Promise.all([
                    ...
                    // merging model environment with permutation environment
